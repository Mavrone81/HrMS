          ...
          <div className="bg-white rounded-[3rem] shadow-2xl w-full max-w-4xl border border-slate-100 flex flex-col max-h-[90vh] animate-in slide-in-from-top-10 overflow-hidden">
            ...
            <div className="p-10 overflow-y-auto space-y-12">
              <div className="grid grid-cols-3 gap-8">
                <div className="bg-indigo-50/50 p-8 rounded-[2rem] border border-indigo-100/50"><p className="text-[9px] font-black text-indigo-600 uppercase tracking-widest mb-3">Gross Liquidity</p><h4 className="text-3xl font-black text-slate-900 tracking-tighter">{reviewRunData.net}</h4></div>
                <div className="bg-emerald-50/50 p-8 rounded-[2rem] border border-emerald-100/50"><p className="text-[9px] font-black text-emerald-600 uppercase tracking-widest mb-3">Statutory Burden</p><h4 className="text-3xl font-black text-slate-900 tracking-tighter">{reviewRunData.cpf}</h4></div>
                <div className="bg-slate-50/50 p-8 rounded-[2rem] border border-slate-100"><p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-3">Active Population</p><h4 className="text-3xl font-black text-slate-900 tracking-tighter">{reviewRunData.employees}</h4></div>
              </div>
                      ...
                      <tbody className="divide-y divide-slate-50">
                        {filteredEmployees.map((emp, i) => (
                          <tr key={i} className="hover:bg-slate-50 cursor-pointer transition-all" onClick={() => setSelectedEmployee(emp)}>
                            <td className="px-8 py-4"><div className="flex flex-col"><span className="font-black text-slate-900 uppercase tracking-tight">{emp.name}</span><span className="text-[10px] font-bold text-slate-400 uppercase mt-0.5">{emp.department}</span></div></td>
            ...
            <div className="bg-slate-50 border-t border-slate-100 p-10 flex justify-between items-center rounded-b-[3rem]">
               <button onClick={() => { const id = reviewRunData.id; setReviewRunData(null); setRuns(runs.map(r => r.id === id ? { ...r, status: 'Rejected' } : r)); handleActionToast('Protocol Rejected. Operational cycle reset.'); }} className="px-10 py-5 bg-white border border-red-200 text-red-600 text-[10px] font-black uppercase tracking-widest rounded-2xl hover:bg-red-50 transition-all shadow-xl shadow-red-500/5 active:scale-95">Reject Protocol</button>
               <div className="flex gap-4">
                 ...
                 <button onClick={() => { const id = reviewRunData.id; setReviewRunData(null); setRuns(runs.map(r => r.id === id ? { ...r, status: 'Disbursed' } : r)); handleActionToast('Settlement Approved. Liquidity synchronized.'); }} className="px-12 py-5 bg-indigo-600 text-white text-[10px] font-black uppercase tracking-widest rounded-2xl hover:bg-indigo-700 shadow-2xl shadow-indigo-500/20 active:scale-95 transition-all">Authorize Disbursement</button>
               </div>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-slate-950/40 backdrop-blur-md p-8 animate-in fade-in duration-300">
           <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-3xl overflow-hidden border border-slate-100 animate-in slide-in-from-bottom-5">
              <div className="p-10 border-b border-slate-100 flex justify-between items-center">
                 <div><h3 className="text-2xl font-black text-slate-900 tracking-tighter uppercase leading-none">Entity Payslips</h3><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-3">Fiscal Window: {payslipPeriod}</p></div>
              ...
              <div className="max-h-[50vh] overflow-y-auto">
                 <table className="w-full text-left text-sm border-collapse">
                    <thead className="bg-slate-50/50 text-[10px] font-black text-slate-400 uppercase tracking-widest sticky top-0 z-10 backdrop-blur-md">
                       <tr><th className="px-10 py-5">Personnel Name</th><th className="px-10 py-5">Structural Unit</th><th className="px-10 py-5 text-right">Net Settlement</th><th className="px-10 py-5 text-center">Export Protocol</th></tr>
                    </thead>
                    <tbody className="divide-y divide-slate-50">
                       {mockEmployees.slice(0, 8).map((emp, i) => (
                          <tr key={i} className="hover:bg-slate-50 transition-all group">
                             ...
                             <td className="px-10 py-4 text-center"><button onClick={() => handleActionToast(`Encrypting PDF for ${emp.name}…`)} className="px-4 py-1.5 bg-indigo-50 text-indigo-600 text-[9px] font-black uppercase rounded-lg hover:bg-indigo-600 hover:text-white transition-all">Download PDF</button></td>
                          </tr>
                       ))}
                    </tbody>
                 </table>
